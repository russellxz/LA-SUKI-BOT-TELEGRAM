// plugins/Slap.js — le dio una cachetada a a alguien del grupo (con ranking)
import fs from "fs";
...
const handler = async (msg, ctx) => {
  ...
  await conn.sendMessage(chatId, {
    text:
      `🖕 ${mencion(msg.senderId, msg.senderName)} le dio una cachetada a a ${mencion(objetivo.id, objetivo.nombre)}\n\n` +
      `📊 Lleva *${db[grupo][objetivo.id]}* cachetadas en este grupo.`,
    mentions: [msg.senderId, objetivo.id]
  }, { quoted: msg });
};

handler.command = ["slap", "cachetada"];
